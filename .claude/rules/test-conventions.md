...
- Jest env is `jsdom` (set in `package.json`); `babel-jest` transforms via `babel.config.cjs`.
...
- `tests/integration/` now exists (`tests/integration/smoke.test.js`); new integration tests go there and run via `npm run test:integration`.
