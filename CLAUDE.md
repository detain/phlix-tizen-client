...
**Entry**: `index.html` (repo root, the Vite root) loads `/src/main.ts`, which mounts into `#phlix-app`, `#phlix-spatial-host`, `#phlix-chapter-overlay`, `#phlix-skip-intro-overlay`, `#phlix-pip-overlay`, `#phlix-action-toast-overlay` (S516 AD-13), `#phlix-quick-connect` (S520 AD-25), and `#phlix-telemetry-consent` (S521 AD-27).
...
- **`main.ts`** — boot, `createPhlixApp` config, `TIZEN_HLS_CONFIG` (bounded buffers, `capLevelToPlayerSize`, `enableSoftwareAES`), `buildMenu()` / `buildExtraRoutes()` (S517 T-10: the admin nav entry + the `buildAdminRoutes()` spread are gated behind the default-off `VITE_PHLIX_TV_ADMIN === '1'` build flag — unset means no admin route exists on TV; measured scope: drops the admin-layout chunk, page chunks stay linked via @phlix/ui's static registry import (full exclusion + CSS split = ui follow-ups), 2nd-app SpatialNavHost mount, 3rd–8th-app overlay mounts (`ChapterOverlay`, `SkipIntroOverlay`, `PiPController`, `ActionToastOverlay` — the S516 transient action caption, presentational, never focusable, key-dismissed — plus `QuickConnectPanel` (S520) and `TelemetryConsent` (S521)), `installGamepadBridge()` (S522 dev/QA gamepad→key), and `wireHubRelayConsumer` (which, since S510, maps the relay's transient `waiting-visible` status to a single auto-dismissing `@phlix/ui` toast — non-modal, cleared on recovery).
...
Vitest + `jsdom` + `@vue/test-utils` (`vitest.config.ts`, `npm test`). Tests live in `tests/unit/*.test.ts` and are co-located by module name (the `src/` tree is flat). Setup `tests/test-setup.ts` provides an in-memory localStorage mock. Coverage (v8) excludes `app/**`, `tests/`, `dist/`, configs and is threshold-enforced in CI (statements 72 / branches 61 / functions 68 / lines 73 — pinned at the measured floor after the T-11 suites landed, S501 T-13; ratchet up, never down). Current suites (36 files): `resolveConfig`, `deviceId`, `polyfills`, `tizenBridge` (pure `wireTizenBridge` exercised with fakes), `SpatialNavHost`, `RemoteManager`, `KeyMapping`, `registerKeys` (S509, fake-tizen register/teardown pins), `gamepadBridge` (S522, pure-poll deadzone/400-150-repeat/A-B + idempotent install + absent-API no-op + real-`RemoteManager` BACK integration + focus-safe source grep), `ChapterOverlay`, `SkipIntroOverlay`, `SubtitleTrackList`, `useMusicStore`, `useTrackPreferenceStore` (S511, fake-`ApiClient` GET/PUT settings tuples + per-item `localStorage`), `languageLadder` (S511, pure per-item→server→none ordering), `RatingBadge`, `RatingModal`, `UserRatingPicker`, `useSyncPlayStore`, `syncPlayWireShape`, `syncPlayMigration`, `hubRelay`, `syncplayDispatch`, `quickconnect` (S520 transport+session), `QuickConnectPanel` (S520 wiring), `telemetry` (S521 consent-gate+cadence+swallow), `TelemetryConsent` (S521 card seam), `RouteWireShape`, `TrackWireShape`, `TrackApplyBoundary`, `TrackLanguageMemory` (S511, both track pages mounted with Pinia + fake `@phlix/ui` ApiClient/player store), `ParentalControlsWireShape`, `routeManifest.gate`, and `main` (mocks `@phlix/ui`/`@phlix/contracts`/`vue` and asserts the boot wiring). SFC suites mount the component with `@phlix/ui`'s `ApiClient`/`useApiBase`/`usePlayerStore` and `vue-router` stubbed via `vi.hoisted` + `vi.mock`.
...
- **Each overlay needs a host `<div>`** — adding another mounted app means adding its container to `index.html` as well as the `createApp(...).mount(...)` call in `main.ts` (the boot now runs EIGHT root apps: the six above plus `#phlix-quick-connect` (7th) and `#phlix-telemetry-consent` (8th)).
