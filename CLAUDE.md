...
- **`main.ts`** — boot, `createPhlixApp` config, `TIZEN_HLS_CONFIG` (bounded buffers, `capLevelToPlayerSize`, `enableSoftwareAES`), `buildMenu()` / `buildExtraRoutes()`, 2nd-app SpatialNavHost mount, 3rd–6th-app overlay mounts (`ChapterOverlay`, `SkipIntroOverlay`, `PiPController`, `ActionToastOverlay` — the S516 transient action caption, presentational, never focusable, key-dismissed), and `wireHubRelayConsumer` (which, since S510, maps the relay's transient `waiting-visible` status to a single auto-dismissing `@phlix/ui` toast — non-modal, cleared on recovery).
