...
- **`main.ts`** — boot, `createPhlixApp` config, `TIZEN_HLS_CONFIG` (bounded buffers, `capLevelToPlayerSize`, `enableSoftwareAES`), `buildMenu()` / `buildExtraRoutes()` (S517 T-10: the admin nav entry + the `buildAdminRoutes()` spread are gated behind the default-off `VITE_PHLIX_TV_ADMIN === '1'` build flag — unset means no admin route exists on TV; measured scope: drops the admin-layout chunk, page chunks stay linked via @phlix/ui's static registry import (full exclusion + CSS split = ui follow-ups), 2nd-app SpatialNavHost mount, 3rd–9th-app overlay mounts (`ChapterOverlay`, `SkipIntroOverlay`, `PiPController`, `ActionToastOverlay` — the S516 transient action caption, presentational, never focusable, key-dismissed — plus `QuickConnectPanel` (S520), `TelemetryConsent` (S521), and `ScreenSaverOverlay` (S523 idle screensaver; the keep-awake leg is WITHHELD — no `privilege/display` in the manifest and TN-2/S503 forbids pre-adding)), `installGamepadBridge()` (S522 dev/QA gamepad→key), and `wireHubRelayConsumer` (which, since S510, maps the relay's transient `waiting-visible` status to a single auto-dismissing `@phlix/ui` toast — non-modal, cleared on recovery).
